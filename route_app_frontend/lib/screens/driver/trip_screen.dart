        ...
        if (_currentPosition != null)
          Positioned(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: const Color(0xFF101524).withValues(alpha: 0.85),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: AppTheme.border),
              ),
              child: Row(
...
class _PodBottomSheetState extends ConsumerState<_PodBottomSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_photoFile != null) {
        uploadedPhotoUrl = await api.uploadStopPhoto(widget.tripId, widget.stop.id, _photoFile!);
      }

      await api.updateTripStop(widget.tripId, widget.stop.id, {
        'status': _selectedStatus,
        'receiver_name': _receiverController.text.trim().isEmpty ? null : _receiverController.text.trim(),
        'signature_data': _signatureBase64,
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text('Tap to take photo', style: TextStyle(color: Colors.grey.shade500, fontSize: 13)),
                          ],
                        ),
                      ),
                    ),
